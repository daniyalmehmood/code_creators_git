public class Repeater  {
    public static String repeatFront(String str, int n) {
        StringBuilder front = new StringBuilder(); // Create a StringBuilder object to build 'front' variable
        ...
        if (!str.isEmpty() && str.length() >= 3 && n > 0)  // the 'str' should be not empty, the 'str' length is more than or equal 3 && the 'n' is positive
            ...
            length = 3; // make the length is 3
                front.append(str, 0, 3);  //assign to 'front' to be the first 3 element from the 'str'
                for (int i = 0; i < n; i++) //use a for loop to go through the 'front'
                {
                    repeatedValue.append(front); // add the 'front' to 'repeatedValue' after repeated 'n' times
                }
            }
